...
export default function Update8AExtractScreen() {
    ...
    const handleSendOtp = async () => {
        ...
        if (!khataNumber) return Alert.alert("Error", "Enter Khata Number");
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            data.append("khata_number", khataNumber);
            data.append("account_number", khataNumber);
            data.append("district", district);
            data.append("taluka", taluka);
            data.append("village", village);
            data.append("full_name", correctedName || "N/A");
            ...
            const res = await axios.post(API_ENDPOINTS.LAND_8A_CORRECTION_SUBMIT, data, {
    ...
    const getNewValueDisplay = () => {
        switch (selectedType) {
            ...
            case "other": return otherDetails || "See Docs";
        ...
        return (
            <View style={s.root}>
                ...
                <SafeAreaView style={s.successContainer}>
                    ...
                    <Text style={s.successSubtitle}>Your 8A Extract correction request has been submitted successfully.</Text>
                    <View style={s.idCard}>
                    ...
                    <TouchableOpacity style={s.mainBtn} onPress={() => router.replace("/8a-extract-services")}>
            ...
            <SafeAreaView style={s.safe}>
                <View style={s.header}>
                    ...
                    <View style={s.headerContent}>
                        <Text style={s.headerTitle}>8A Extract Correction</Text>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
                        ...
                        {step === 1 && (
                            <View>
                                ...
                                <View style={s.card}>
                                    <Text style={s.inputLabel}>Khata Number *</Text>
                                    <InputGroup icon="book-outline" placeholder="Enter Khata Number" value={khataNumber} onChangeText={setKhataNumber} />
                            ...
                            <View>
                                ...
                                {selectedType && (
                                    <View style={s.card}>
                                        ...
                                        {selectedType === "name" && <InputGroup label="Correct Full Name *" icon="person-outline" placeholder="Enter new name" value={correctedName} onChangeText={setCorrectedName} />}
                                        {selectedType === "area" && <InputGroup label="Correct Land Area (Hectors) *" icon="expand-outline" placeholder="Ex: 0.45.00" value={correctedArea} onChangeText={setCorrectedArea} />}
                                        {selectedType === "occupant" && <InputGroup label="Correct Occupant Details *" icon="people-outline" placeholder="Enter details" value={correctedOccupant} onChangeText={setCorrectedOccupant} multiline style={{ height: 60 }} />}
                                        {selectedType === "land_use" && <InputGroup label="Correct Land Use *" icon="leaf-outline" placeholder="Ex: Agricultural to NA" value={correctedLandUse} onChangeText={setCorrectedLandUse} />}
                                ...
                                <View style={s.card}>
                                    ...
                                    {selectedType && <DocUpload label="Correction Supporting Document *" id="supporting_doc" file={uploadedDocs.supporting_doc} onUpload={handleFileUpload} onRemove={(id: string) => setUploadedDocs(p => { const n = { ...p }; delete n[id]; return n; })} />}
                                </View>
                        ...
                        {step === 3 && (
                            <View>
                                ...
                                <View style={s.card}>
                                    <ReviewRow label="Khata Number" value={khataNumber} />
                                    <ReviewRow label="Location" value={`${village}, ${taluka}, ${district}`} />
                                    <ReviewRow label="Correction Type" value={selectedType?.replace('_', ' ').toUpperCase() || ""} />
