...
const HousingApartmentServicesScreen = () => {
    ...
    const [societyRegId, setSocietyRegId] = useState("");
    const [mobileNumber, setMobileNumber] = useState("");
    const [paymentAmount, setPaymentAmount] = useState("");
    const [isConfirmed, setIsConfirmed] = useState(false);

    // UI States
    const [societySearchQuery, setSocietySearchQuery] = useState(""); // Used for Location
    const [isLoading, setIsLoading] = useState(false);
    const [duesDetails, setDuesDetails] = useState<HousingDetails | null>(null);
    ...
    const validateForm = () => {
        if (selectedSociety.trim().length < 3) return false;
        if (societySearchQuery.trim().length < 3) return false; // societySearchQuery is now Location
        if (flatNumber.trim().length === 0) return false;
        if (mobileNumber.trim().length !== 10) return false;
        return true;
    };

    const handleFetchDues = async () => {
        if (selectedSociety.trim().length < 3) {
            Alert.alert("Required", "Please enter valid Society Name");
            return;
        }
        if (societySearchQuery.trim().length < 3) {
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerCenter}>
                        <Text style={styles.headerTitle}>Housing / Apartment</Text>
                        <Text style={styles.headerSubtitle}>Pay maintenance and charges securely</Text>
                    </View>
                                ...
                                <>
                                    ...
                                    <View style={styles.formCard}>
                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Society / Apartment Name *</Text>
                                            <View style={styles.inputContainer}>
                                                ...
                                                <TextInput
                                                    ...
                                                    onChangeText={setSelectedSociety}
                                                />
                                            </View>
                                        </View>

                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Society / Apartment Location *</Text>
                                            <View style={styles.inputContainer}>
                                        ...
                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Flat Number / Plot Number *</Text>
                                            <View style={styles.inputContainer}>
                                        ...
                                        <View style={styles.row}>
                                            <View style={[styles.fieldGroup, { flex: 1, marginRight: 10 }]}>
                                                <Text style={styles.fieldLabel}>Wing / Building</Text>
                                                <View style={styles.inputContainer}>
                                                    <MaterialCommunityIcons name="domain" size={16} color="#94A3B8" />
                                                    <TextInput style={styles.input} placeholder="e.g. Wing A" placeholderTextColor="#94A3B8" value={wing} onChangeText={setWing} />
                                                </View>
                                            </View>
                                            <View style={[styles.fieldGroup, { flex: 1 }]}>
                                                <Text style={styles.fieldLabel}>Society Reg. ID</Text>
                                                <View style={styles.inputContainer}>
                                                    <MaterialCommunityIcons name="card-account-details-outline" size={16} color="#94A3B8" />
                                                    <TextInput style={styles.input} placeholder="Reg. ID" placeholderTextColor="#94A3B8" value={societyRegId} onChangeText={setSocietyRegId} />
                                                </View>
                                            </View>
                                        </View>
                                ...
                                <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
                                    ...
                                    <View style={styles.summaryCard}>
                                        ...
                                        <View style={styles.summaryRow}>
                                            ...
                                            <Text style={styles.summaryValue}>{duesDetails?.residentName}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Flat No.</Text>
                                            <Text style={styles.summaryValue}>{duesDetails?.flatNumber}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                        ...
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Maint. Charges</Text>
                                            <Text style={styles.summaryValue}>₹{duesDetails?.maintenanceCharges}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Parking</Text>
                                            <Text style={styles.summaryValue}>₹{duesDetails?.parkingCharges}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Water</Text>
                                            <Text style={styles.summaryValue}>₹{duesDetails?.waterCharges}</Text>
                                        </View>

                                        <View style={styles.amountBanner}>
                                            <View>
                                                <Text style={styles.bannerLabel}>Penalty</Text>
                                                <Text style={styles.bannerValue}>₹{duesDetails?.penalty}</Text>
                                            </View>
                                    ...
                                    <TouchableOpacity style={styles.declarationRow} onPress={() => setIsConfirmed(!isConfirmed)}>
                                        ...
                                        <Text style={styles.declarationText}>I confirm that the above apartment details are correct and authorize this payment.</Text>
                                    </TouchableOpacity>
                    ...
                    <View style={styles.successOverlay}>
                        <View style={styles.successCard}>
                            ...
                            <View style={styles.receipt}>
                                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>Transaction ID</Text><Text style={styles.receiptValue}>TX-HSG-{Math.floor(Math.random() * 900000) + 100000}</Text></View>
                                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>Society Name</Text><Text style={styles.receiptValue}>{selectedSociety}</Text></View>
                                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>Flat Number</Text><Text style={styles.receiptValue}>{flatNumber}</Text></View>
                            ...
                            <TouchableOpacity style={styles.backHomeButton} onPress={() => { setShowPaymentSuccess(false); router.back(); }}><Text style={styles.backHomeText}>Back to Housing Services</Text></TouchableOpacity>
                        </View>
                    </View>
                </Modal>
            </SafeAreaView>
...
export default HousingApartmentServicesScreen;
