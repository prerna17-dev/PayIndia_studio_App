...
type PropertyCorrectionType = "owner_name" | "property_area" | "property_type" | "other";
...
export default function PropertyTaxUpdateScreen() {
    ...
    const [selectedType, setSelectedType] = useState<PropertyCorrectionType | null>(null);
    ...
    const handleSendOtp = async () => {
        ...
        if (!propertyId) return Alert.alert("Error", "Enter Property ID");

        try {
            const token = await AsyncStorage.getItem("userToken");
            const res = await axios.post(
                API_ENDPOINTS.PROPERTY_TAX_OTP_SEND,
                { mobile_number: mobileNumber, aadhaar_number: aadhaarNo },
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (correctedName) data.append("corrected_owner_name", correctedName);
            if (correctedArea) data.append("corrected_property_area", correctedArea);
            if (correctedType) data.append("corrected_property_type", correctedType);
            if (otherDetails) data.append("other_details", otherDetails);

            if (uploadedDocs.id_proof) {
                data.append("id_proof", { uri: uploadedDocs.id_proof.uri, name: uploadedDocs.id_proof.name, type: "image/jpeg" } as any);
            }
            ...
            if (uploadedDocs.other_doc) {
                data.append("other_doc", { uri: uploadedDocs.other_doc.uri, name: uploadedDocs.other_doc.name, type: "application/pdf" } as any);
            }

            const res = await axios.post(API_ENDPOINTS.PROPERTY_TAX_CORRECTION_SUBMIT, data, {
        ...
        return (
            <View style={s.root}>
                ...
                <SafeAreaView style={s.successContainer}>
                    ...
                    <Text style={s.successSubtitle}>Your property tax correction request has been submitted successfully.</Text>
                    <View style={s.idCard}>
                    ...
                    <TouchableOpacity style={s.mainBtn} onPress={() => router.replace("/property-tax-services")}>
            ...
            <SafeAreaView style={s.safe}>
                <View style={s.header}>
                    ...
                    <View style={s.headerContent}>
                        <Text style={s.headerTitle}>Property Tax Correction</Text>
                        <Text style={s.headerSubtitle}>मालमत्ता कर दुरुस्ती</Text>
                    </View>
                    <View style={{ width: 40 }} />
                </View>
                            ...
                            <View>
                                <View style={s.sectionHeader}>
                                    ...
                                    <View>
                                        ...
                                        <Text style={s.sectionSub}>Verify existing Property detail</Text>
                                    </View>
                                </View>

                                <View style={s.card}>
                                    <Text style={s.inputLabel}>Property ID / Tax No. *</Text>
                                    <InputGroup icon="business-outline" placeholder="Enter Property ID" value={propertyId} onChangeText={setPropertyId} />
                                    <Text style={s.inputLabel}>Owner Aadhaar Number *</Text>
                                    <InputGroup icon="card-outline" placeholder="Enter 12-digit Aadhaar" keyboardType="numeric" maxLength={12} value={aadhaarNo} onChangeText={formatAadhaar} />
                        ...
                        {step === 2 && (
                            <View>
                                <View style={s.sectionHeader}>
                                    ...
                                    <View><Text style={s.sectionTitle}>Correction Type</Text><Text style={s.sectionSub}>Select field to correct</Text></View>
                                </View>
                                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.typeList}>
                                    {([
                                        { id: "owner_name", label: "Owner Name", icon: "person" },
                                        { id: "property_area", label: "Area Size", icon: "square" },
                                        { id: "property_type", label: "Usage Type", icon: "business" },
                                        { id: "other", label: "Other", icon: "ellipsis-horizontal" },
                                    ] as { id: PropertyCorrectionType; label: string; icon: string }[]).map((t) => (
                                ...
                                {selectedType && (
                                    <View style={s.card}>
                                        ...
                                        {selectedType === "owner_name" && <InputGroup label="Correct Owner Name *" icon="person-outline" placeholder="Enter full name" value={correctedName} onChangeText={setCorrectedName} />}
                                        {selectedType === "property_area" && <InputGroup label="Correct Area (Sq.Ft) *" icon="square-outline" placeholder="Enter size" value={correctedArea} onChangeText={setCorrectedArea} keyboardType="numeric" />}
                                        {selectedType === "property_type" && <InputGroup label="Correct Usage Type *" icon="business-outline" placeholder="Residential/Commercial" value={correctedType} onChangeText={setCorrectedType} />}
                                        <InputGroup label="Details/Notes" icon="document-text-outline" placeholder="Optional explanation" value={otherDetails} onChangeText={setOtherDetails} multiline style={{ height: 60 }} />
                                    </View>
                                )}
                                <Text style={s.stepDesc}>📄 DOCUMENTS REQUIRED – CORRECTION PROOF</Text>
                                <View style={s.card}>
                                    <View style={s.sectionHeader}>
                                        <View style={[s.iconBadge, { backgroundColor: '#E3F2FD' }]}><Ionicons name="document-text" size={20} color="#0D47A1" /></View>
                                        <View><Text style={s.sectionTitle}>Required Proofs</Text><Text style={s.sectionSub}>Supporting documents</Text></View>
                                    </View>
                                    ...
                                    <DocUpload label="Other Documents" id="other_doc" file={uploadedDocs.other_doc} onUpload={handleFileUpload} onRemove={(id: string) => setUploadedDocs(p => { const n = { ...p }; delete n[id]; return n; })} />
                                </View>
                        ...
                        {step === 3 && (
                            <View>
                                <View style={s.sectionHeader}>
                                    ...
                                    <View><Text style={s.sectionTitle}>Summary</Text><Text style={s.sectionSub}>Review correction request</Text></View>
                                </View>
                                <View style={s.card}>
                                    <ReviewRow label="Property ID" value={propertyId} />
                                    <ReviewRow label="Correction For" value={selectedType?.replace('_', ' ').concat(' Update').toUpperCase()} />
                                    <ReviewRow label="Updated Value" value={correctedName || correctedArea || correctedType || "Multiple changes"} isGreen />
                                    <ReviewField label="ID Proof" value={uploadedDocs.id_proof ? "Uploaded ✅" : "Not Provided"} color={uploadedDocs.id_proof ? "#2E7D32" : "#64748B"} />
                                    <ReviewField label="Correction Proof" value={uploadedDocs.supporting_doc ? "Uploaded ✅" : "Not Provided"} color={uploadedDocs.supporting_doc ? "#2E7D32" : "#64748B"} />
                                    <ReviewField label="Other Docs" value={uploadedDocs.other_doc ? "Uploaded ✅" : "Not Provided"} color={uploadedDocs.other_doc ? "#2E7D32" : "#64748B"} />
                                </View>
...
const DocUpload = ({ label, id, file, onUpload, onRemove }: any) => (
    <View style={{ marginBottom: 12 }}>
        ...
        {file ? (
            ...
            <TouchableOpacity style={s.uploadBox} onPress={() => onUpload(id)}><Ionicons name="cloud-upload-outline" size={20} color="#0D47A1" /><Text style={s.uploadText}>Upload Document</Text></TouchableOpacity>
        )}
    </View>
);

const ReviewField = ({ label, value, color }: { label: string, value: string, color?: string }) => (
    <View style={s.reviewRow}>
        <Text style={s.reviewLabel}>{label}</Text>
        <Text style={[s.reviewVal, color ? { color } : {}]}>{value}</Text>
    </View>
);
