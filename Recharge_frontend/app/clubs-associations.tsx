...
const membershipTypes = ["Annual", "Lifetime", "Monthly", "Event-Based"];

const ClubsAssociationsScreen = () => {
    ...
    const [duesDetails, setDuesDetails] = useState<ClubDuesDetails | null>(null);
    ...
    const validateForm = () => {
        if (selectedClub.trim().length < 3) return false;
        if (clubLocation.trim().length < 3) return false;
        if (membershipId.trim().length === 0) return false;
        if (mobileNumber.trim().length !== 10) return false;
        if (!membershipType) return false;
        return true;
    };

    const handleFetchDues = async () => {
        if (selectedClub.trim().length < 3) {
            Alert.alert("Required", "Please enter valid Club / Association Name");
            return;
        }
        if (clubLocation.trim().length < 3) {
            Alert.alert("Required", "Please enter valid Location");
            return;
        }
        if (!membershipType) {
            Alert.alert("Required", "Please select Membership Type");
            return;
        }
        ...
        setIsLoading(true);
        setTimeout(() => {
            const mockData: ClubDuesDetails = {
                memberName: memberName || "Vikram Singh",
                membershipId: membershipId,
                associationName: selectedClub,
                period: "Yearly Subscription 2026",
            ...
            <SafeAreaView style={styles.safeArea}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.headerCenter}>
                        <Text style={styles.headerTitle}>Clubs & Associations</Text>
                        <Text style={styles.headerSubtitle}>Pay membership fees and association charges</Text>
                    </View>
                                ...
                                <>
                                    ...
                                    <View style={styles.formCard}>
                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Club / Association Name *</Text>
                                            <View style={styles.inputContainer}>
                                                <MaterialCommunityIcons name="office-building" size={16} color="#94A3B8" />
                                                <TextInput
                                                    style={styles.input}
                                                    placeholder="Enter Club Name"
                                                    placeholderTextColor="#94A3B8"
                                                    value={selectedClub}
                                                    onChangeText={setSelectedClub}
                                                />
                                            </View>
                                        </View>

                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Club / Association Location *</Text>
                                            <View style={styles.inputContainer}>
                                        ...
                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Membership ID / Number *</Text>
                                            <View style={styles.inputContainer}>
                                        ...
                                        <View style={styles.fieldGroup}>
                                            <Text style={styles.fieldLabel}>Member Name (Optional)</Text>
                                            <View style={styles.inputContainer}>
                                        ...
                                        <View style={styles.row}>
                                            ...
                                            <View style={[styles.fieldGroup, { flex: 1 }]}>
                                                ...
                                                <TouchableOpacity style={styles.inputContainer} onPress={() => setShowTypeModal(true)}>
                                                    <MaterialCommunityIcons name="account-details-outline" size={16} color="#94A3B8" />
                                                    <Text style={[styles.input, { color: membershipType ? '#333' : '#94A3B8' }]}>
                                    ...
                                    <TouchableOpacity onPress={handleFetchDues} disabled={!validateForm() || isLoading} style={{ marginBottom: 24 }}>
                                        <LinearGradient
                                            colors={!validateForm() || isLoading ? ["#E0E0E0", "#E0E0E0"] : ["#0D47A1", "#1565C0"]}
                                            start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                                            style={styles.actionButton}
                                        >
                                            {isLoading ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.actionButtonText}>Fetch Dues</Text>}
                                        </LinearGradient>
                                    </TouchableOpacity>
                                </>
                            ) : (
                                <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
                                    ...
                                    <View style={styles.summaryCard}>
                                        <View style={styles.summaryHeader}>
                                            <MaterialCommunityIcons name="card-bulleted-outline" size={24} color="#0D47A1" />
                                            <Text style={styles.summaryTitle}>Dues Summary</Text>
                                        </View>
                                        <View style={styles.divider} />

                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Member</Text>
                                            <Text style={styles.summaryValue}>{duesDetails.memberName}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>ID</Text>
                                            <Text style={styles.summaryValue}>{duesDetails.membershipId}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Association</Text>
                                            <Text style={styles.summaryValue}>{duesDetails.associationName}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Period</Text>
                                            <Text style={styles.summaryValue}>{duesDetails.period}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Subscription</Text>
                                            <Text style={styles.summaryValue}>₹{duesDetails.subscriptionFee}</Text>
                                        </View>
                                        <View style={styles.summaryRow}>
                                            <Text style={styles.summaryLabel}>Event Charges</Text>
                                            <Text style={styles.summaryValue}>₹{duesDetails.eventCharges}</Text>
                                        </View>

                                        <View style={styles.amountBanner}>
                                            <View>
                                                <Text style={styles.bannerLabel}>Late Fee</Text>
                                                <Text style={styles.bannerValue}>₹{duesDetails.lateFee}</Text>
                                            </View>
                                            <View style={styles.verticalDivider} />
                                            <View>
                                                <Text style={styles.bannerLabel}>Total Payable</Text>
                                                <Text style={styles.bannerValue}>₹{duesDetails.totalPayable}</Text>
                                            </View>
                                        </View>
                                    </View>
                                    ...
                                    <TouchableOpacity style={styles.declarationRow} onPress={() => setIsConfirmed(!isConfirmed)}>
                                        ...
                                        <Text style={styles.declarationText}>I confirm that the above membership details are correct and authorize this payment.</Text>
                                    </TouchableOpacity>

                                    <View style={styles.footerButtons}>
                                        <TouchableOpacity style={styles.cancelButton} onPress={() => setDuesDetails(null)}>
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={[styles.modalContent, { height: 'auto', paddingBottom: 40 }]}>
                            <View style={styles.modalHeader}>
                                ...
                                <TouchableOpacity onPress={() => setShowTypeModal(false)}><Ionicons name="close" size={24} color="#666" /></TouchableOpacity>
                            </View>
                            <View style={styles.optionsList}>
                                {membershipTypes.map((type: string, index: number) => (
                                    <TouchableOpacity key={index} style={styles.optionItem} onPress={() => { setMembershipType(type); setShowTypeModal(false); }}>
                                        <Text style={styles.optionText}>{type}</Text>
                                        {membershipType === type && <Ionicons name="checkmark-circle" size={20} color="#0D47A1" />}
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </View>
                    ...
                    <View style={styles.successOverlay}>
                        <View style={styles.successCard}>
                            ...
                            <View style={styles.receipt}>
                                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>Transaction ID</Text><Text style={styles.receiptValue}>TX-CLC-{Math.floor(Math.random() * 900000) + 100000}</Text></View>
                                <View style={styles.receiptRow}><Text style={styles.receiptLabel}>Membership ID</Text><Text style={styles.receiptValue}>{membershipId}</Text></View>
                            ...
                            <TouchableOpacity style={styles.backHomeButton} onPress={() => { setShowPaymentSuccess(false); router.back(); }}><Text style={styles.backHomeText}>Back to Clubs & Associations</Text></TouchableOpacity>
                        </View>
                    </View>
                </Modal>
            </SafeAreaView>
...
export default ClubsAssociationsScreen;
