...
exports.sendOTP = async (req, res, next) => {
    try {
        ...
        await NCLModel.storeOTP(mobile_number, otpCode, "NCL_APPLY");

        const SmsService = require("../services/sms.service");
        await SmsService.sendSMS(mobile_number, `Your OTP for NCL Certificate Verification (Aadhaar: ****${aadhar_number.slice(-4)}) is ${otpCode}. Valid for 10 mins.`);
...
exports.verifyOTP = async (req, res, next) => {
    try {
        ...
        const isValid = await NCLModel.verifyOTP(mobile_number, otp_code, "NCL_APPLY");
